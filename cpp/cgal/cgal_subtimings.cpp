...
std::ostream& operator<<(std::ostream& os, const std::array<TElem, 2>& vec) {
...
std::vector<std::array<double, 2>> calculate_alpha_shape(std::list<Point> &points, std::vector<Segment> &segments, double alpha=1.0, int n=1)
{
  std::vector<std::array<double, 2>> time_list;
  ...
  for (int i = 0; i< n; i++)
  {
    ...
    Triangulation_2 delaunay_triangulation;
    // Time delaunay triangulation
    auto t1 = std::chrono::high_resolution_clock::now();
    delaunay_triangulation.insert(points.begin(), points.end());
    auto t2 = std::chrono::high_resolution_clock::now();
    // Time Alpha Shape Creation
    Alpha_shape_2 A(delaunay_triangulation,
                    FT(alpha),
                    Alpha_shape_2::REGULARIZED);
    alpha_edges(A, std::back_inserter(segments_temp)); // this takes about 2 ms for a 64,000 point cloud
    auto t3 = std::chrono::high_resolution_clock::now();
    double triangulation = std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() * 1e-3;
    double alpha_shape = std::chrono::duration_cast<std::chrono::microseconds>(t3 - t2).count() * 1e-3;

    time_list.push_back({{triangulation, alpha_shape}});
  }
  ...
  // Timings indicate no difference between REGULARIZED VS GENERAL MODE for the input data in these tests
  return time_list;
}
